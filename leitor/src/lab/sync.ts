export const DOCUMENT = 'laboratorio-anotacoes-v1'
export type AnnotationRecord = { annotationId: string; pageIndex: number; payload: string | null; revision: number; operationId: string }
export type Pending = Omit<AnnotationRecord, 'revision'> & { baseRevision: number; document: string; queuedAt: number }
...
export class AnnotationSync {
  ...
  constructor(owner: string, storage: Storage, save: (pending: Pending) => Promise<SaveResult>, changed: (state: SyncStatus) => void, document: string = DOCUMENT) {
    ...
    } catch { this.error = 'Não foi possível recuperar os rascunhos locais. Não feche esta página.' }
  }
  get drafts() { return this.pending }
  get hasPending() { return this.pending.length > 0 }
  get hasConflict() { return this.conflictRevision !== null }
  hasPendingFor(id: string) { return this.pending.some(p => p.annotationId === id) }
  revisionFor(id: string) { return this.revisions.get(id) ?? 0 }
  ...
  private persist(op: Pending) {
    try { this.storage.setItem(this.prefix + op.operationId, JSON.stringify(op)) }
    catch { this.error = 'Sem espaço para o rascunho local. Mantenha a página aberta até sincronizar.' }
  }
  async flush() {
    ...
    try {
      while (this.pending.length && !this.stopped) {
        ...
        if (result.status === 'conflict') { this.conflictRevision = result.revision; break }
    ...
    } catch { this.error = 'Alterações pendentes. Reconecte ou tente sincronizar novamente.' }
  ...
  private emit() { if (!this.stopped) this.changed({ pending: this.pending.length, error: this.error, conflict: this.hasConflict }) }
}
