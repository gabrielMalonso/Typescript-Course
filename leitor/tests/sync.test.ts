...
import { AnnotationSync, type Pending } from '../src/pdf/sync'
...
const DOCUMENT = 'test-reading@v1'
const settled = () => new Promise(resolve => setTimeout(resolve, 0))

describe('annotation outbox', () => {
  it('restores failed writes after reload, without mixing owners', async () => {
    const disk = storage()
    const offline = new AnnotationSync('owner', disk, async () => { throw new Error('offline') }, () => {}, DOCUMENT)
    offline.enqueue('stroke', 0, 'original')
    await settled()
    expect(disk.length).toBe(1)
    offline.stop()
    const saved: Pending[] = []
    const restored = new AnnotationSync('owner', disk, async op => { saved.push(op); return { status: 'saved', revision: 1 } }, () => {}, DOCUMENT)
    expect(new AnnotationSync('stranger', disk, vi.fn(), () => {}, DOCUMENT).hasPending).toBe(false)
  ...
  it('serializes rapid edits using consecutive revisions', async () => {
    ...
    const sync = new AnnotationSync('owner', storage(), async op => { operations.push(op); return { status: 'saved', revision: op.baseRevision + 1 } }, () => {}, DOCUMENT)
  ...
  it('keeps independent drafts from two tabs in separate storage entries', async () => {
    ...
    const a = new AnnotationSync('owner', disk, offline, () => {}, DOCUMENT)
    const b = new AnnotationSync('owner', disk, offline, () => {}, DOCUMENT)
    a.enqueue('a', 0, 'A'); b.enqueue('b', 1, 'B')
    await settled()
    expect(disk.length).toBe(2)
    const restored = new AnnotationSync('owner', disk, offline, () => {}, DOCUMENT)
