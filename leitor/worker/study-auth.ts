...
export async function withStudyAccess(request: Request, env: AuthEnv, next: () => Promise<Response>): Promise<Response> {
  ...
  if (url.pathname === '/auth/iniciar') {
    ...
    return redirect(workos.userManagement.getAuthorizationUrl({ provider: 'authkit', redirectUri: url.origin + '/auth/retorno', state: state.nonce, codeChallenge: challenge, codeChallengeMethod: 'S256' }), headers)
  }
